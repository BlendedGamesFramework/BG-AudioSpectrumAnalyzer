...
class AnalyzerViews {
    ...
    AnalyzerGraphic graphView = null;
    ...
    AnalyzerViews(AnalyzerActivity _activity, FirebaseAuth.AuthStateListener  mAuthListener) {
        activity = _activity;
...
//                popupMenuAverage.showAsDropDown(view, 0, 0);
    ...
    private void refreshPeakLabel(double maxAmpFreq, double maxAmpDB) {
        ...
        textPeak.append("Hz(");
        musicalNote = AnalyzerUtil.freq2Cent(textPeak, maxAmpFreq, " ");
        textPeak.append(") ");
        ...
        instance =  WebSocket.getInstance();
        id_player = WebSocket.getId_player();
        if(instance != null){
            try {
                dataJSON = new JSONObject().put("data", textPeak.toString()).put("id_player",id_player).toString();
                instance.emit("message",dataJSON);
